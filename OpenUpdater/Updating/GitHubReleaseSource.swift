...
enum GitHubReleaseSource {
    ...
    static func latest(for recipe: UpdateRecipe) async throws -> ReleaseResult {
        ...
        guard let repo = recipe.check.repo,
              let url = URL(string: "https://api.github.com/repos/\(repo)/releases?per_page=30") else {
        ...
        guard let chosen = releases.first(where: {
            !$0.draft && (recipe.check.prereleases || !$0.prerelease)
        }) else {
            throw UpdateCheckError.noReleases
        }

        return ReleaseResult(tag: chosen.tagName, version: recipe.normalizeVersion(fromTag: chosen.tagName))
    }
}
